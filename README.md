...
> 5. ### Path Traversal
> - Здесь я прибегнул к встроенным возможностям пакета ```net/http```. Добавил файловый сервер для статических файлов, избавился от способа чтения файла по пути из GET параметра ```file```. Насколько я изучил способы защиты, то это не позволит нам переходить по иным путям, так как веб-серверу нечем на них отвечать. Также, хороший способ, использовать базовую директорию и проверять, что запрошенный файл находится в пределах этой директории.
